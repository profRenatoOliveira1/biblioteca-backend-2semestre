...
export class Auth {
    ...
    static async validacaoUsuario(req: Request, res: Response): Promise<any> {
        ...
        const querySelectUser = `SELECT id_usuario, nome, username, imagem_perfil FROM usuario WHERE username=$1 AND senha=$2;`;
    ...
    static generateToken(id: number, nome: string, username: string) {
        ...
        return jwt.sign({ id, nome, username }, SECRET, { expiresIn: '24h' });
    }
